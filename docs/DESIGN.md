...
events.jsonl의 한 줄 = 하나의 이벤트. **두 종류**:
...
| `tool` | enum | `Read` / `Edit` / `Write` / `Glob` / `Grep` / `_control` |
| `path` | string \| undefined | 파일 경로 (tool 이벤트). `_control`에는 없음 |
| `ext` | string \| null \| undefined | 확장자. `path.extname(path).toLowerCase()`로 자동 도출. Glob/Grep은 `null`. `_control`에는 없음 |
| `event` | enum \| undefined | `_control`에서만 사용: `session_start` / `session_end` / `compact` |
...
claude-trail은 **4종 hook**을 등록함: `PostToolUse`, `SessionStart`, `SessionEnd`, `PreCompact`.
모두 같은 hook 진입점(`dist/hook.js`)이 받고, `hook_event_name` 필드로 분기.
...
| `PostToolUse: Task` (subagent) | ❌ | subagent의 PostToolUse가 별도 라벨링 필요. v0.2 |
| `PostToolUse: WebFetch`, `WebSearch` | ❌ | "파일 상호작용" 정의 밖 |
| `UserPromptSubmit` | ❌ | 너무 빈번. v0.2 옵트인 |
| `PreToolUse`, `Stop`, `SubagentStop`, `Notification` | ❌ | 정보 가치 낮거나 중복 |
...
2. **M1 — 데이터 파이프라인**
   ...
   - `hook.ts`: stdin → JSONL append — **별도 진입점**, React/Ink 미참조
     - `hook_event_name` 분기로 4종(`PostToolUse`/`SessionStart`/`SessionEnd`/`PreCompact`) 처리 (§5.1)
