...
- **상태:** Draft v1.0 (2026-05-07) — M0.5 실측 반영
- **저자:** 오경택
- **위치:** repo 루트 (standalone repo)
- **라이선스:** MIT

> **v1.0 변경사항 (M0.5 실측 결과):**
> - Task tool 실제 이름은 **`Agent`** (DESIGN의 가정 `Task`와 다름) — events.jsonl에는 `Task`로 정규화
> - 서브에이전트 attribution은 공식 필드 **`agent_id`/`agent_type`** 존재 → §5.1.3 **Case A 확정**
> - `/compact` 시퀀스가 3-step (PreCompact → SubagentStop with `agent_type:""` → SessionStart with `source:"compact"`)
> - SessionStart `source` enum에 `"compact"` 추가
> - PostToolUse 페이로드에 `permission_mode`, `tool_use_id`, `duration_ms` 발견 — `tool_use_id`는 dedup 키로 채택
> - SubagentStop 페이로드는 `subagent_session_id` 대신 `agent_id`/`agent_type`/`agent_transcript_path`/`last_assistant_message`
...
**(c) Subagent 호출 이벤트** — Task tool 호출 (Claude Code 내부 이름은 `Agent`,
events.jsonl에는 `Task`로 정규화):
```json
{
  ...
  "meta": {
    ...
    "tool_use_id": "toolu_01JyYwq8WnXDfSD2MgJERYRp",
    "duration_ms": 5135
  }
}
```
...
`meta.agent_id`/`meta.agent_type`이 부착되어 부모와 attribute됨 (§5.1.3 — M0.5
실측으로 **Case A 확정**, 휴리스틱 매칭 불필요). 서브에이전트는 부모와 **같은
`session` 값**을 가짐.

| 필드 | 타입 | 설명 |
|------|------|------|
| `ts` | ISO8601 (UTC, ms) | 이벤트 시각. 항상 `Z` 접미사. 표시 단계에서 로컬 변환 |
| `session` | string | Claude 세션 ID (서브에이전트도 부모와 동일 — M0.5 실측) |
| `tool` | enum | `Read` / `Edit` / `Write` / `Glob` / `Grep` / `Task` / `_control` (`Task`는 Claude Code 내부 `Agent`를 정규화한 값) |
...
PostToolUse hook은 Claude Code로부터 stdin에 다음 형태의 JSON을 받음 (M0.5 실측):
...
```json
{
  ...
  "tool_response": { "type": "text", "file": { "filePath": "...", "content": "...", "numLines": 200, "totalLines": 500, "startLine": 1 } },
  "tool_use_id": "toolu_01K5uLhNJPbqXv1md5LxASDR",
  "duration_ms": 1,
  "agent_id": "a7d9ed34b488363a3",
  "agent_type": "Explore"
}
...
`agent_id`/`agent_type`은 **서브에이전트의 tool 호출에서만** 옴 (부모 세션의 직접 호출엔 없음). §5.1.3 attribution 키.

각 tool 매핑 규칙 (`tool_name` 컬럼은 stdin의 raw 값, `tool` 컬럼은 events.jsonl 정규화 값):

| stdin tool_name | events.jsonl tool | path | meta | 비고 |
|-----------------|-------------------|------|------|------|
| `Read` | `Read` | `tool_input.file_path` (상대경로 변환) | `{ offset, limit, lines, tool_use_id, duration_ms }` | `tool_response.file.content` **즉시 폐기**. `numLines`만 `meta.lines`로 발췌 (§12) |
| `Edit` | `Edit` | `tool_input.file_path` | `{ replace_all, tool_use_id, duration_ms }` | `old_string`/`new_string` 저장 안 함 |
| `Write` | `Write` | `tool_input.file_path` | `{ bytes: content.length, tool_use_id, duration_ms }` | `content` 본문 저장 안 함 |
| `Glob` | `Glob` | `tool_input.path \|\| "."` | `{ pattern, tool_use_id, duration_ms }` | path는 검색 루트 |
| `Grep` | `Grep` | `tool_input.path \|\| "."` | `{ query: pattern, glob, type, tool_use_id, duration_ms }` | 검색어는 사용자 의도 추적용으로 보존 (§12) |
| `Agent` | **`Task`** | (없음) | `{ subagent_type, description, agent_id, tool_use_id, duration_ms }` | `tool_input.prompt` **저장 안 함**. `description`이 비어있을 때 prompt 첫 80자 프리뷰 fallback. `tool_response`의 풍부한 metric(totalTokens 등)은 v0.2 |
...
Claude Code가 보내는 페이로드와 매핑 (M0.5 실측 반영):

| hook_event_name | 추가 필드 | → events.jsonl 라인 |
|-----------------|---------|--------------------|
| `SessionStart` | `source: "startup"\|"resume"\|"clear"\|"compact"`, `model` | `{tool:"_control", event:"session_start", meta:{source, model}}` |
| `SessionEnd` | `reason: "clear"\|"logout"\|"prompt_input_exit"\|"other"` | `{tool:"_control", event:"session_end", meta:{reason}}` |
| `PreCompact` | `trigger: "manual"\|"auto"`, `custom_instructions: string\|null` | `{tool:"_control", event:"compact", meta:{trigger}}` (custom_instructions는 프라이버시상 미저장) |

**`/compact` 시퀀스 — 3-step (M0.5 실측):**
1. `PreCompact (trigger:"manual"\|"auto")` — 압축 시작
2. `SubagentStop (agent_type:"")` — Claude Code가 내부 sub-LLM으로 요약 생성
3. `SessionStart (source:"compact")` — 압축된 컨텍스트로 세션 재시작

**TUI 표시 정책:** events.jsonl엔 3건 모두 기록하지만, **TUI에서는 PreCompact 한 줄로 흡수**.
직후 발생한 `agent_type:""`인 SubagentStop과 `source:"compact"`인 SessionStart는
PreCompact의 부산물이므로 별도 라인 표시 안 함 (§7).
...
**M0.5 실측 결론 — Case A 확정.** Claude Code의 PostToolUse/SubagentStop 페이로드는
공식 필드 `agent_id` (string) 와 `agent_type` (string)을 포함. 휴리스틱 불필요.

**Task 호출(부모 컨텍스트):**
- 부모 세션이 `Agent` tool을 호출 → `PostToolUse(tool_name="Agent")` 발화 → §5.1.1
  매핑으로 `{tool:"Task", session:<parent>, meta:{subagent_type, description, agent_id, tool_use_id, duration_ms}}` 한 줄.
- `agent_id`는 Claude Code가 발급한 식별자 (예: `"a7d9ed34b488363a3"`). attribution 키.
- `tool_response.agentId`와 PostToolUse `tool_input.subagent_type`는 같은 객체의 다른 필드.

**서브에이전트 종료:**

| hook_event_name | 추가 필드 (M0.5 실측) | → events.jsonl |
|-----------------|---------|---------------|
| `SubagentStop` | `agent_id`, `agent_type`, `agent_transcript_path`, `last_assistant_message`, `stop_hook_active` | `{tool:"_control", event:"subagent_stop", session:<parent>, meta:{agent_id, agent_type, transcript_path}}` (`last_assistant_message`는 §12 미저장) |

**서브에이전트 내부 tool 호출 attribution (M0.5 Case A 확정):**

서브에이전트의 자체 tool 호출은 **부모와 동일 `session_id`** + `agent_id`/`agent_type`
필드로 PostToolUse 발화. 매핑:
- `{tool:"Read"|..., session:<parent>, meta:{..., agent_id, agent_type}}` — meta에 attribution 부착
- TUI는 `meta.agent_id`가 있으면 한 단계 들여쓰기 + `[<agent_type>]` 라벨
- agent_id로 Task 호출 ↔ 자식 tool 호출 ↔ SubagentStop 정확 매칭

**`/compact` 부산물 SubagentStop (M0.5 발견):**
`/compact` 처리 시 Claude Code가 내부 sub-LLM으로 요약 → SubagentStop 발화 (`agent_type:""`,
`agent_id`는 있음). 이 이벤트는 **events.jsonl엔 정상 기록**하지만 `agent_type === ""`일 때
TUI에서 PreCompact 라인에 흡수 (§7).

**Nested 서브에이전트(Task → Task):** Case A 확정으로 v0.1은 **1단계 들여쓰기로 평탄화**.
2단계 이상은 같은 들여쓰기 깊이에 `[parent▸child]` 결합 라벨. 깊이별 들여쓰기는 v0.2.

**입력 크기 한계 + cwd 결정 우선순위:**
- stdin은 buffered 1-shot read (Claude가 `tool_response` 큰 본문도 보낼 수 있음 — Read의
  `tool_response.file.content`에 파일 전문이 옴, M0.5 실측 확인).
...
- 프로젝트 루트(§15)를 결정할 cwd는 다음 우선순위:
  ...
  3) `process.cwd()`.
- **Dedup 정책 (M0.5 변경):** `tool_use_id`가 페이로드에 항상 포함됨이 확인됨.
  v0.1부터 **(session, tool_use_id) 키로 in-process dedup 적용**: hook이 동일 키 재진입 시
  no-op. Claude의 retry/재발화 중복 방지. 디스크 기반 dedup(crash 후)은 v0.2.
...
- Stream에서 일반 라인이 아니라 **가로 구분선** 형식으로 강조:
  ...
  - `─── HH:MM:SS  session start (clear|resume|startup|compact) ───`
...
**`/compact` 부산물 흡수 (M0.5 발견):**
`/compact` 처리 시 Claude Code가 PreCompact → SubagentStop(`agent_type:""`) → SessionStart(`source:"compact"`)
시퀀스를 발화. TUI는 PreCompact 한 줄로 흡수하고 후속 2건은 표시 생략:
- `agent_type === ""`인 SubagentStop: 직전 PreCompact과 같은 session_id면 흡수
- `source === "compact"`인 SessionStart: 직전 PreCompact과 같은 session_id면 흡수
- events.jsonl엔 모두 기록 (단일 source of truth 유지)

**서브에이전트 표시 (M0.5 Case A 확정):**
- Task 호출 라인: `TASK   ⮕ <subagent_type>: "<description>"` (brightBlue + bold).
- 서브에이전트의 내부 tool 호출: **2 spaces 들여쓰기 + `↳ ` 마커 + `[<agent_type>]` 우측 라벨**.
  attribution은 페이로드의 `agent_id`/`agent_type` 직접 매칭 (휴리스틱 X).
- `subagent_stop` 이벤트: `   ✓ <agent_type> done · <duration> · <N> calls` (green + dim).
  단 `agent_type === ""`이면 /compact 부산물이라 흡수.
...
`.claude/settings.json` (M0.5 검증된 형태):
```json
{
  "hooks": {
    "PostToolUse": [
      {
        ...
        "hooks": [
          { "type": "command", "command": "node $CLAUDE_PROJECT_DIR/dist/hook.js" }
        ]
      }
    ],
    "SubagentStop": [
...
**매처 동작 (M0.5 관찰):** 설정한 matcher 문자열에 `Agent`가 없어도 PostToolUse가
`tool_name="Agent"`로 발화함이 관찰됨 (headless `claude -p` 환경). 매처 의미가
우리 가정과 다를 가능성. 안전책으로 **hook 내부에서도 tool_name 화이트리스트 필터** 적용.
v0.2 추가 검증.

**Hook 명령 경로 정책:** v0.1은 standalone 모드 — `node $CLAUDE_PROJECT_DIR/dist/hook.js`로
실행. `CLAUDE_PROJECT_DIR`은 Claude Code가 자동 설정. v0.3 글로벌 설치(`npm i -g`)
시 `claude-trail-hook` 바이너리로 단순화.
...
- ~~서브에이전트 attribution 휴리스틱~~ → M0.5 실측으로 공식 필드(`agent_id`/`agent_type`) 확인, **Case A로 v0.1 포함**.
...
1. **M0 — TypeScript 부트스트랩**
   ...
   - `dist/`까지 빌드 동작 확인
2. **M0.5 — Hook payload 실측 (✅ 완료, 2026-05-07)**
   - capture 모드 hook(`CLAUDE_TRAIL_CAPTURE_DIR` env로 stdin 그대로 dump)을 settings.json에 5종 등록.
   - 실제 Claude Code 세션 (interactive + headless `claude -p` 혼합) 으로 14건 페이로드 캡처.
   - **확정 사항:**
     - `agent_id` / `agent_type` 공식 필드 존재 → §5.1.3 **Case A 채택**
     - Task tool 실제 이름 `Agent` → events.jsonl에 `Task`로 정규화
     - `tool_use_id` 항상 포함 → dedup 키로 채택
     - `/compact` 3-step 시퀀스 → §5.1.3 흡수 정책
     - PostToolUse 추가 필드 (`permission_mode`, `duration_ms`) 활용
   - **미실측:** `/clear` 시퀀스(Q9) — 별도 인터랙티브 시나리오 필요. M1 후 검증.
3. **M1 — 데이터 파이프라인**
   - `cli.ts` 골격 (M0 완료, watch/init은 미구현 stub)
   - `hook.ts`: stdin → JSONL append — **별도 진입점**, React/Ink 미참조
     - `hook_event_name` 분기로 5종 처리 (§5.1)
     - PostToolUse: `tool_name` 화이트리스트 (`Read|Edit|Write|Glob|Grep|Agent`) 필터, Agent → Task 정규화
     - `agent_id`/`agent_type` 부착 (Case A attribution)
     - `(session, tool_use_id)` 키 in-process dedup (§5.1.3)
   - `lib/paths.ts`: 프로젝트 루트 결정 + 절대→상대 정규화 (§15)
   - `lib/events.ts`: HookPayload → TrailEvent 매퍼
   - **node:test 단위 테스트** (M0.5 fixture 활용):
     - `tests/fixtures/captured-*.jsonl` (14건 실측 페이로드)
     - mapper 출력 검증 (각 hook 종류별)
     - `/compact` 시퀀스 흡수 정책 검증
     - 절대경로 정규화 + outside flag 검증
   - 100ms 회귀 측정 (`time` 명령)
4. **M2 — TUI 골격**
   - Ink + JSX(.tsx) 셋업
...
> M0.5 실측으로 Q9/Q11/Q12 해소, Q7/Q14(신규) 부분 해소.
...
- **Q7.** Hook 입력의 `cwd`가 프로젝트 루트인지 cwd인지 케이스가 다를 수 있음.
  - **M0.5 부분 해소:** stdin payload의 `cwd`가 항상 사용자 working directory와 일치 관찰됨.
    `env_pwd` 환경변수와도 일치. §5.1.3 우선순위(payload.cwd → CLAUDE_PROJECT_DIR → process.cwd())로 충분.
- **Q8.** Init 시 사용자 확인(`y`) 없이 비대화 모드(`--yes`)가 필요한가?
  - 가설: v0.2에서 추가. v0.1은 항상 대화형.
- **Q9.** ~~`/clear`가 실제로 `SessionEnd(reason:"clear")` + `SessionStart(source:"clear")` 순서로 발화되는가?~~
  - **M0.5에선 미실측** (`/clear`는 별도 인터랙티브 시나리오 필요). M1 후 검증.
  - **부분 발견:** `/compact`는 PreCompact → SubagentStop(`agent_type:""`) → SessionStart(`source:"compact"`) 시퀀스 (DESIGN가정과 다름, §5.1.2/§5.1.3 갱신).
- **Q10.** `PreCompact`의 `custom_instructions` 저장 — 사용자 의도 추적 가치 있지만
  프라이버시 우려.
  - **M0.5 부분 해소:** 실측 시 `custom_instructions: null`로 옴 (사용자가 instruction
    안 줬을 때). 어차피 v0.1 미저장 정책 — 변동 없음.
- **Q11.** ~~Subagent attribution: Claude Code hook 페이로드에 `parent_session_id` 같은 공식 필드가 있는가?~~
  - **✅ M0.5 해소.** 공식 필드 `agent_id` (string) + `agent_type` (string)가 PostToolUse / SubagentStop 페이로드에 존재. **Case A 확정**, 휴리스틱 불필요.
- **Q12.** ~~동시에 여러 서브에이전트가 돌 때 휴리스틱이 잘못 매칭하면?~~
  - **✅ M0.5 해소.** Q11 결과로 휴리스틱 자체가 사라져 misattribution 위험 0.
- **Q13.** Task의 `prompt` 본문 저장 옵션 (`init --include-task-prompts`)?
  - 가설: v0.2 옵트인 + 1 KB cap.
- **Q14 (신규).** Hook matcher 의미 — 설정 matcher에 `Agent`가 없어도 PostToolUse가
  `tool_name="Agent"`로 발화 (M0.5 관찰).
  - 가설: matcher 의미가 우리 가정과 다르거나 `claude -p` headless에서만 적용 안 됨.
    안전책으로 hook 내부에서 화이트리스트 필터 (§11). v0.2 추가 검증.
- **Q15 (신규).** Read의 `tool_response.file.numLines` 등 파일 메타 보존 범위?
  - 가설: v0.1은 `numLines`만 `meta.lines`로 발췌(§5.1.1). 본문은 즉시 폐기.
- **Q16 (신규).** Agent의 `tool_response.totalDurationMs` / `totalTokens` / `toolStats` 활용?
  - 가설: v0.1은 메타에 미포함, v0.2에서 SubagentBlock의 done 라인에 노출 (`5.1s · 4 calls · 34k tokens`).
