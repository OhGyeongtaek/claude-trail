...
| `ext` | string \| null | 확장자. `path.extname(path).toLowerCase()`로 자동 도출. Glob/Grep은 `null` (검색 루트는 디렉터리이므로) |
| `meta` | object | tool별 부가정보 (§5.1). 비어 있을 수 있음 |
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node ./tools/claude-trail/dist/hook.js"
          }
        ]
      }
    ]
...
2. **M1 — 데이터 파이프라인**
   ...
   - `hook.ts`: stdin → JSONL append (§5.1 매핑) — **별도 진입점**, React/Ink 미참조
   - `lib/paths.ts`: 프로젝트 루트 결정 (§15)
   - 수동 검증: `echo '{"hook_event_name":"PostToolUse","tool_name":"Read","tool_input":{"file_path":"a.ts"}}' | node dist/hook.js`
